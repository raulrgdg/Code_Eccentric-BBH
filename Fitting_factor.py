...
#Problema con los bounds; para e>0.7 el best parameter q es todo lo pequeño que el bound permita, siempre tiende al valor mínimo
#esto significa que para excentricidades grandes, la mejor plantilla correponde a aquella donde una masa es mucho mayor que la otra: m1>>m2


'''
Problema:
usando e=0.1; la repoducibilidad no es exacta, ay que obtenemos diferentes resultaods pero que cambian apenas del orden del 6/7 decimal
siempre alrededor del máximo, por lo que podemos aproximar que no cometemos error

pero usanod e>0.1, la reproducibilidad se vuelve exageradamente caótica, cada resutlado puede variar mucho en q o mchirp, variando mucho en el segundo 
decimal del overlap (como si se estancara en maximos locales). puedo fijar la reproducibilidad con random.sedd, pero 
no se como escoger el caso máximo

Creo que se puede deber a que cuando e>0.1 la zona del máximo es más indistinguible, y por tanto, es más dificl encontrar
el máximo global

Por que tarda menos?? esta relacionado?? como controlarlo??? -- buscar. #LA CLAVE ERA POPSIZE, SI SUBO HASTA 50 NO OBTENGO MAXIMOS LOCALES!!!!!!!!!!!!!!! 

#Just to show the constraints of the script - not reproducibility
overlaps=[]
for i in range(10):
    q,mchirp,overlap,t=maxoverlap_sig_tem(0.5,6.08364,0.2)
    overlaps.append(-overlap)

print('The obtained overlaps, in the same conditions, are:\n ', overlaps)
'''
